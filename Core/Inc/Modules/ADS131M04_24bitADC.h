...
class ADS131M04_24bitADC: public AbstractModule
{
	public:
		ADS131M04_24bitADC(const STRHAL_SPI_Id_t &spiId, const STRHAL_SPI_Config_t &spiConf, const STRHAL_GPIO_t &dataReadyPin);
	...
	private:
		bool readReg(const ADSAddr &address, uint8_t *reg, uint8_t n = 0);
		bool writeReg(const ADSAddr &address, uint8_t reg, uint16_t delay = 0);
