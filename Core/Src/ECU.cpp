...
int ECU::init() {
	...
	if(GenericChannel::init() != 0)
		return -1;
	/*
	LID_QSPI_Config_t w25q256_conf = {0};
	w25q256_conf.flash_size = 24;
	w25q256_conf.fmode = LID_QSPI_FMODE_SPI;
	w25q256_conf.ncs_high_time = 7;
	w25q256_conf.psc = 7;
	w25q256_conf.clk_level = 0;
	LID_QSPI_Flash_Init(&w25q256_conf);*/
...
int ECU::exec() {
	...
	while(1) {
		if(imu_0.exec() < 0) {
			LID_UART_Write("IMU FAIL\n", 5);
			return -1;
		}

		if(imu_0.getMeas(x)) {
			std::sprintf(kartoffel_buffer, "%d,%d,%d,%d,%d,%d,%d\n", x.temp, x.accel.x, x.accel.y, x.accel.z, x.alpha.x, x.alpha.y, x.alpha.z);
