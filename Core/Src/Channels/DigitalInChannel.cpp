...
DigitalInChannel::DigitalInChannel(uint8_t channel_id, const STRHAL_GPIO_t &digin_pin, uint32_t refresh_divider)
	: AbstractChannel(CHANNEL_TYPE_ADC16, channel_id, refresh_divider), digin_pin(digin_pin) {
}

int DigitalInChannel::init() {
	STRHAL_GPIO_SingleInit(&digin_pin,STRHAL_GPIO_TYPE_IHZ);
...
int DigitalInChannel::getSensorData(uint8_t *data, uint8_t &n) {
	...
	*out = (STRHAL_GPIO_Read(&digin_pin) == STRHAL_GPIO_VALUE_L) ? 0UL : 1UL;
...
uint16_t DigitalInChannel::getMeas() const {
	return (STRHAL_GPIO_Read(&digin_pin) == STRHAL_GPIO_VALUE_L) ? 0UL : 1UL;;
}
