...
{ [STRHAL_TIM_TIM6] =
{ .timx = TIM6, .utype = STRHAL_TIM_USAGE_000, .burnie = NULL, }, [STRHAL_TIM_TIM7] =
{ .timx = TIM7, .utype = STRHAL_TIM_USAGE_000, .burnie = NULL, }, };
static STRHAL_TIM_Timer_t _tims[STRHAL_TIM_N_TIM] =
{ [STRHAL_TIM_TIM1] =
{ .timx = TIM1, .ch_n = 4, .utype = STRHAL_TIM_USAGE_000, }, [STRHAL_TIM_TIM2] =
{ .timx = TIM2, .ch_n = 4, .utype = STRHAL_TIM_USAGE_000, }, [STRHAL_TIM_TIM3] =
{ .timx = TIM3, .ch_n = 4, .utype = STRHAL_TIM_USAGE_000, }, [STRHAL_TIM_TIM4] =
...
int32_t STRHAL_TIM_PWM_Read(STRHAL_TIM_PWM_Channel_t *pwmChannel, uint16_t *duty)
{
	...
	*duty = (uint16_t) ((*ch->ccr) & 0xFFFF);
	uint16_t freq = SystemCoreClock / (LL_TIM_GetAutoReload(ch->tim->timx) * (LL_TIM_GetPrescaler(ch->tim->timx) + 1));
	return freq;
}
